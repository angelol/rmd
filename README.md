...
## Advantages
* This utility will only delete direct subdirectories of the current working directory.
* It will never traverse to the parent directory or delete the current directory itself.
* It only allows you to delete one directory at a time.
* It asks for confirmation before deletion and only proceeds on explicit `y` / `Y` (default is **no**).
